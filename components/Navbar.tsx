...
const Navbar: React.FC<NavbarProps> = ({ darkMode, toggleDarkMode, language, setLanguage }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 dark:bg-institutional-dark/90 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className={`lg:hidden p-2 rounded-full transition-colors ${isScrolled ? 'text-slate-700 dark:text-slate-200' : 'text-white'}`}
          >
            {isMobileMenuOpen ? (
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white dark:bg-institutional-dark border-t border-slate-200 dark:border-slate-800">
          <div className="px-6 py-4 space-y-3">
            {[
              ...
              <Link 
                ...
                className="block py-2 text-slate-700 dark:text-slate-200 hover:text-institutional-gold transition-colors font-medium"
              >
                {item.label}
              </Link>
            ))}
            <Link to="/investir" onClick={() => setIsMobileMenuOpen(false)}>
              <button className="w-full mt-4 bg-institutional-gold hover:bg-yellow-600 text-institutional-blue font-bold px-6 py-3 rounded transition-all">
                Investir
              </button>
            </Link>
          </div>
