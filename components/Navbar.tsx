    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 dark:bg-institutional-dark/90 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-institutional-blue rounded flex items-center justify-center">
            {/* Logo agrandi et rogné via SVG */}
            <img src="/assets/logo-priorites-rdc.svg" alt="Priorités RDC" className="w-12 h-12 object-cover rounded" />
          </div>
        ...
        <div className="hidden lg:flex items-center gap-8 font-medium">
          {['Stratégie', 'Observatoire', 'Projets', 'Actualités', 'Partenaires'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className={`transition-colors hover:text-institutional-gold ${isScrolled ? 'text-slate-700 dark:text-slate-200' : 'text-white'}`}>
              {item}
            </a>
          ))}
        </div>
